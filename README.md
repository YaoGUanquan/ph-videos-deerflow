...
基于 [DeerFlow 2.0](https://github.com/bytedance/deer-flow) 的视频生成项目，整合脚本生成、视频生成（火山/通义/ComfyUI）等能力。
...
- **Docker 部署**：使用 `config.docker.yaml`（Docker 沙箱），或复制为 `config.yaml` 后编辑
- **exe / 本地模式**：使用 `config.example.yaml` 复制为 `config.yaml`，选择 Local 沙箱
...
- **ph-videos-deerflow**：本项目，基于 DeerFlow + Docker/Electron，支持云 + 桌面
